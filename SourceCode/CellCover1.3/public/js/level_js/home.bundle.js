/***2020.04.28***/ (function(modules) { // webpackBootstrap
    ...
    exportExcel: function exportExcel(json) {
        if (json["links"].length) {
            ...
            convertData.forEach(function (d, i) {
                ...
                convertDataExtract[i]["上层缺陷"] = d["source"]["id"];
                convertDataExtract[i]["下层缺陷"] = d["target"]["id"];
            });
    ...
    rectLayout: function rectLayout(json) {
        if (json["nodes"].length) {
            ...
            d3.selectAll('.node').each(function (d, i) {
                endCordinates['x' + i] = $('.graph-area').width() * 1 / 4 + count4row * ($('.graph-area').width() * 3 / 4 / sqr);
                endCordinates['y' + i] = $('.graph-area').height() * 1 / 5 + row * ($('.graph-area').height() * 4 / 5 / (sqr + 2));
                if (count4row < sqr - 1) {
                    count4row++;
                } else {
                    count4row = 0;
    ...
    var rps = {
        ...
        "result": {
            "nodes": [
				...
				{ "id": "TOF", "group": ["项遗漏缺陷"] },
				{ "id": "LNF", "group": ["词取反缺陷"] },
				{ "id": "CORF", "group": ["或操作符取反缺陷"] },
				{ "id": "DORF", "group": ["与操作符取反缺陷"] },
				{ "id": "TNF", "group": ["项取反缺陷"] },
				{ "id": " ", "group": [" "] },
				{ "id": " ", "group": [" "] },
				{ "id": "ENF", "group": ["表达式取反缺陷"] } 
